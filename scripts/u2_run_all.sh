...
say "U2 START repo=$(git rev-parse --short HEAD) steps=$STEPS"
...
run_arm() {   # $1 slug, $2 arm (C|I)
  local slug="$1" arm="$2"
  local out="$ROOT/outputs/u2_${slug}_arm${arm}_seed42"
  ...
  if [ "$arm" = "C" ]; then
    "$PY" -m ppg2ecg.training.train_a0 --exp-name "u2_${slug}_armC_seed42" --out-dir "$out" \
    ...
    "$PY" -m ppg2ecg.training.train_a2 --exp-name "u2_${slug}_armI_seed42" --out-dir "$out" \
      --processed "$proc" --manifest "$man" "${COMMON[@]}" \
