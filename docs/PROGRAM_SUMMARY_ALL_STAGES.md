...
커밋·푸시됐습니다. WildPPG 테스트 피험자 `kjd`/`ssx`는 A4 · A5c · A6c · A9 · X0 · X2에서 동결 A4 split의 **테스트로 평가에 쓰였고**, X3-G0부터는 모든 단계에서 **로드되지 않았습니다** (X3-G0 사전 설계 감사가 기존 테스트 배열을 읽은 사실은 `X3_G0_PREPREREG_DESIGN_AUDIT.md`에 공개). *2026-09-13 정정: 초판은 "전 구간 미로드"로 잘못 적었음.*
...
- 테스트 778개 통과, `external/PENGUIN` 전 구간 무수정, `kjd`/`ssx`는 X3-G0부터 미로드 (A4~X2는 테스트로 평가)
