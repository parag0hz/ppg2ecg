...
| Time inputs | `h` only (code) / `(t, h)` (MF paper) | **`cond = E(t) + E(1000·h)`, `h = t − r`, using the backbone's single existing `TimestepEmbedder` for both (shared weights → parameter count unchanged; the ×1000 scale makes h/r decodable, R² 1.00 vs 0.18 unscaled — review finding)** | keeps the baseline's `t`-conditioning path; `E(h)` adds the interval information without new parameters. `E(t)+E(h)` is symmetric in (t, h); within 0 ≤ h ≤ t ≤ 1 the only self-collisions are the trivial r = 0 points, so no valid (r, t) pairs alias. h-only (official) noted as an alternative, not adopted |
