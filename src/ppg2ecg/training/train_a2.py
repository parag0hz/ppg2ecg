    ...
    try:
        ...
        for epoch in range(state["epoch"], args.epochs):  # "epoch" == validation round (== true epoch unless --val-every-steps)
            ...
            if is_best:
                ...
                torch.save({"state_dict": net.state_dict(), "epoch": epoch, "objective": "improved_meanflow", "selection": {"criterion": "fixed_imf_mse", "value": sel, "min_delta": args.min_delta}, "model_cfg": dict(n_step=1, sample_rate=args.sample_rate, h_dim=args.h_dim, ssm_block_num=args.blocks, ssm_ratio=args.ssm_ratio, mlp_ratio=args.mlp_ratio, **arch_kw), "target_norm": {"mu": tnorm.mu, "sigma": tnorm.sigma, "source": tnorm.source}, "imf_cfg": dict(tr_kw, norm_p=args.norm_p, norm_eps=args.norm_eps, jvp_mode=args.jvp_mode, cond_mode=args.cond_mode, h_scale=args.h_scale), "args": vars(args), "seed": args.seed, "git": meta["git"], "upstream_commit": UPSTREAM_COMMIT}, best_ckpt)
                event = "best"
            else:
                state["no_improve"] += 1
            state["epoch"] = epoch + 1
