...
IN, OUT = ROOT / "outputs/sr1_eval", ROOT / "artifacts/rf1_reflow"
MET = ("HR", "Rpeak_F1", "RR_MAE_ms", "MAE", "RMSE")


def main():
    OUT.mkdir(parents=True, exist_ok=True)
    D = {a: dict(np.load(IN / f"arm_{a}_seed42.npz")) for a in ("C", "I", "S", "R")}
    pid = D["C"]["pid"]
    ...
    verdict = {"RF_vs_iMF": "MIXED" if imp and wor else "IMPROVES" if imp else "WORSE" if wor else "NO MEANINGFUL CHANGE",
               ...
               "teacher_seconds": json.load(open(ROOT / "outputs/rf1_vitaldb_armR_seed42/training_summary.json"))["teacher_seconds"],
               "train_seconds": json.load(open(ROOT / "outputs/rf1_vitaldb_armR_seed42/training_summary.json"))["total_train_time_s"]}
    ...
    for r in rows:
        if r["arm"] == "R" or r["metric"] in ("HR", "Rpeak_F1", "HR_consensus_K16", "FD_kanflow"):
