...
| Time inputs | `h` only (code) / `(t, h)` (MF paper) | **`cond = E(h)`, `h = t − r`, through the backbone's single existing `TimestepEmbedder` (official code's h-only design; parameter count unchanged)**. Tried and rejected before any result: `E(t)+E(h)` (interval nearly invisible, r decodable R² 0.18) and `E(t)+E(1000·h)` (resolves r but amplifies the JVP term 1000×, diverged) | keeps the baseline's `t`-conditioning path; `E(h)` adds the interval information without new parameters. `E(t)+E(h)` is symmetric in (t, h); within 0 ≤ h ≤ t ≤ 1 the only self-collisions are the trivial r = 0 points, so no valid (r, t) pairs alias. h-only (official) noted as an alternative, not adopted |
