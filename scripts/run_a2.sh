...
nohup "$ROOT/.venv/bin/python" -m ppg2ecg.training.train_a2 --out-dir "$OUT" --exp-name "$EXP" --seed 42 --patience 20 --min-delta 1e-4 --epochs 300 --n-val-banks 4 --bank-seed 1000 --gen-diag-every 1 --gen-diag-windows 128 --processed data/processed/v0_8s --manifest data/manifests/split_p0_holdout_seed42.json "$@" > "$OUT/train.log" 2>&1 &
echo $! > "$OUT/train.pid"
